...
BOOL DLLDIR WINAPI HndPoll ( IN HNDID hndid, IN size_t max_skts, OUT LPDWORD active_sockets )
{
	...
	hnd_stop_handler=OpenEvent(SYNCHRONIZE|EVENT_MODIFY_STATE,TRUE,"stop_handler");
	ResetEvent(hnd_stop_handler);

	do
	{
		//"Stop Handler" button was pushed ?
		Ans = WaitForSingleObject(hnd_stop_handler, 0); //no timeout only test event 
		
		//Start Of Test has been sent from handler ?
		sprintf(c_Cmd,"STAT:QUES:EVENT?"); //query the questionable event register
		ibwrt(hndid,c_Cmd,strlen(c_Cmd));
		strset(reply,'\0');
		ibrd(hndid,reply,AnsLength);

	}while( Ans != WAIT_OBJECT_0 && !(atoi(reply) & 0x01) );	//Low level on channel 1 : START TEST DETECTED 

	sprintf(c_Cmd,"*CLS"); //Ensure the value of the questionable event register is cleared
	ibwrt(hndid,c_Cmd,strlen(c_Cmd));

	if(Ans == WAIT_OBJECT_0) //Stop handler requested by user or handler timeout
		active_sockets[0] = 0; 
	else
		active_sockets[0] = 1; 
...
//	Port2 : Bit[0-7]	=> bit[0] = bin 1; bit[1] = bin 2...bit[7] = bin 8
	...
	if(bins[0] <= 0 || bins[0] > 4)
		bins[0] = 4;//default reject bin
