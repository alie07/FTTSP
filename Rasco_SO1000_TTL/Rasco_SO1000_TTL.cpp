...
/*  Layer           : DLL			                                          */
/*                                                                            */
/*  Module          : RASCO_SO1000_TTL                                        */
/*  Description     : Abstraction layer between the RASCO handler and the     */
/*                    FTTSP program                                           */
/*                                                                            */
/*  File            : RASCO_SO1000_TTL.cpp                                    */
/*                                                                            */
/*  Scope           : Public                                                  */
/*                                                                            */
/*  Target          :                                                         */
...
/***************************  REVISION HISTORY  *******************************
 * 
 *    Rev 1.0:   30 Aug 2011 CDESBOUCHAGES
 *    
 * Creation of this DLL according to: 
 * - Software Specification Document DLL RASCO SO1000 Vers A 
 *
 ******************************************************************************/
...
#define MAX_NB_OF_CONFIG_STR		((int)24)								// Number of string into table to configure the 4863
...
#define CMD_READ_QUEST_EVT_REG		"STAT:QUES?"							// Command to read questionable evt register
...
#define MAX_AVAILABLE_SOCKETS		((unsigned int)4)						// RASCO is able to manage 4 sockets
...
static const char kas8ErrorMnemonic[MAX_GPIB_ERR_MESSAGE][5] = {   "EDVR", "ECIC", "ENOL", "EADR", "EARG",
																	"ESAC", "EABO", "ENEB", "EDMA", "",
																	"EOIP", "ECAP", "EFSO", "",     "EBUS",
																	"ESTB", "ESRQ", "",     "",      "",
																	"ETAB", "ELCK", "EARM", "EHDL",  "",
...
//								    "FORMAT:LISTEN HEX",			// Format to listen is in HEX
								    "STAT:QUES:NTR 15",				// Enable the Questionable event reg. on byte1 bit [4..0] (negative transistions)
																	// [SOT |DEV RDY 4|DEV RDY 3|DEV RDY 2|DEV RDY 1]
								    "STAT:QUES:ENAB 16",			// Enable to generate an event on STATUS BYTE REGISTER when SOT PARALLEL event [BIT4] is received.
									...
									"*ESE 0",
									"STAT:OPER:ENAB 0",
									"*SRE 8",						// Enable the service request on questionable event register.
									"*CLS",							// Clear the Status Structure : Do Not Change the Place !!!
 ...
 * FUNCTION NAME : HndInit
 ------------------------------------------------------------------------------
 * FUNCTION DESCRIPTION :
	Attempt to connect to the RASCO handler on GPIB. If it is 
...
{
	HNDID hndDeviceId;		// RASCO handle
	char * ps8Token=NULL;	
	int s32Loop=0;
...
DWORD dwMaskWantedSocket=0;
DWORD dwMaskMaxSockets=0;
DWORD dwInputCondition=0;
DWORD dwCurrentCondition=0;
...
//	ibclr(hndid);
	...
	/* Read the SOT of each DUT */
	if (! HndSetGet(hndid,CMD_READ_QUEST_EVT_REG) )
	{
		PRINT_STDERR("Error on reading QER.\n");
//		ibclr(hndid);
//		HndSet(hndid, kas8MessConfGPIB[0]);
		return FALSE;	
	}
	/* ASCII to int */
	sscanf(as8Buffer, "%d", &dwSiteOnTestMask);
	HndSet(hndid, kas8MessConfGPIB[0]);
	ibclr(hndid);
	
	/* MASK generation of wanted sockets */
	dwMaskWantedSocket=mask_skts;
	/* check if at least one socket is full */
	if ( !(dwSiteOnTestMask&dwMaskWantedSocket) )
	{
		PRINT_STDERR("SOT detected but no socket ready!");
//		ibclr(hndid);
//		HndSet(hndid, kas8MessConfGPIB[0]);
		return FALSE;
	}
	...
	dwMaskWantedSocket&=dwMaskMaxSockets;
	/* generate the requested mask */
	dwSiteOnTestMask&=dwMaskWantedSocket;

	/* Intialization of all bins */
	for (int s32Loop=0; s32Loop<MAX_AVAILABLE_SOCKETS; s32Loop++)
	{
...
//			HndSet(hndid, kas8MessConfGPIB[0]);
	...
	active_sockets[0] = dwSiteOnTestMask;
	return TRUE;
...
char *ps8Buffer = &as8Buffer[1];			// Point onto the nb of test result 
	...
	if (hndid<=0) 
		return FALSE;
	HndSet(hndid, kas8MessConfGPIB[0]);
	//!!LBs:Commented ibclr: Following email from OQa, this seems to be the modification from v1.6 to v1.7:
	//ibclr(hndid);
	...
	while(mask_skts)
	{
		...
		if ( (mask_skts&1)==1)
		{
			...
			dwBin=bins[s32Loop]+1;  // +1 because first bit on cable is bin 0
	...
	Sleep(10);
	HndSetGet(hndid,CMD_READ_QUEST_EVT_REG); // Read(Reset) questionable event register again to avoid bounces effect
	Sleep(40);
 ...
 * FUNCTION NAME : HndSet
 ------------------------------------------------------------------------------
 * FUNCTION DESCRIPTION :
	This function sends a command to the RASCO.
 ------------------------------------------------------------------------------
 * INPUT : hndid
    Comment : RASCO handle 
    Range   : not NULL & >0
 ...
 * FUNCTION NAME : HndGet
 ------------------------------------------------------------------------------
 * FUNCTION DESCRIPTION :
	This function read information from RASCO.
 ------------------------------------------------------------------------------
 * INPUT : hndid
    Comment : RASCO handle 
    Range   : not NULL & >0
 ...
 * FUNCTION NAME : HndGet
 ------------------------------------------------------------------------------
 * FUNCTION DESCRIPTION :
	This function discard the RASCO connection.
 ------------------------------------------------------------------------------
 * INPUT : hndid
    Comment : RASCO handle 
    Range   : not NULL & >0
...
BOOL DLLDIR WINAPI HndRelease ( IN HNDID hndid )
{
	...
	if (ibsta & ERR)
	{
		PRINT_STDERR("Cannot close the RASCO Communication.\n");
		return FALSE;	
	}
	else
	{
